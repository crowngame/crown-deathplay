		...
		if entrance then
			...
			if theFee >= 0 then
				if interiorStatus[INTERIOR_TYPE] == 1 then
					if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) or interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
...
addCommandHandler("moveinterior",
	function(player, cmd, interiorID)
		if (exports.cr_integration:isPlayerLeaderAdmin(player) or exports.cr_integration:isPlayerScripter(player)) then
...
function removeInterior(thePlayer, commandName, houseID)
	if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) or commandName == "MOVETOLS" then
		if not showLoadingProgressTimer then	
...
function cancelRemoveDeletedInts(thePlayer)
	if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) and getElementData(thePlayer, "confirm:removeDeletedInteriors") then	
...
function cancelForceSellAllInactiveInteriors(thePlayer)
	if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) and getElementData(thePlayer, "confirm:forceSellAllInactiveInteriors") then	
...
function cancelRemoveForSaleInts(thePlayer)
	if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) and getElementData(thePlayer, "confirm:removeForSaleInteriors") then	
...
function cancelRemoveInactiveInteriors(thePlayer)
	if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) and getElementData(thePlayer, "confirm:removeInactiveInteriors") then	
...
function changeInteriorName(thePlayer, commandName, ...)
	if (exports.cr_integration:isPlayerSeniorAdmin(thePlayer)) then -- Is the player an admin?
