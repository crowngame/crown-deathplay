...
function showIntName(name, ownerName, inttype, cost, ID, bizMsg)
	...
	elseif name then
		if (inttype==3) then -- Interior name and Owner for rented
			...
			if (exports.cr_integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1) or exports.cr_global:hasItem(getLocalPlayer(), 4, ID) then
			...
			guiLabelSetHorizontalAlign(gInteriorName, "center", true)
			guiSetAlpha(gInteriorName, 0.0)
			if (exports.cr_integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1) or exports.cr_global:hasItem(getLocalPlayer(), 4, ID) or exports.cr_global:hasItem(getLocalPlayer(), 5, ID) then
...
-- local function hasKey(key)
	-- return exports.cr_global:hasItem(getLocalPlayer(), 4, key) or exports.cr_global:hasItem(getLocalPlayer(), 5,key)
-- end

function hasKey(key)
	if exports.cr_global:hasItem(getLocalPlayer(), 4, key) or exports.cr_global:hasItem(getLocalPlayer(), 5,key) then
		return true, false
	else
