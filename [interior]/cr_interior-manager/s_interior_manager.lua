	...
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) then 
		if not tonumber(intID) or (tonumber(intID) <= 0) or (tonumber(intID) % 1 ~= 0) then
			...
			if intID == 0 then
				...
				outputChatBox("Or use KULLANIM: /" .. commandName .. " [Interior ID]", thePlayer, 255, 194, 14)
				return false
			end
		end
		...
		if mQuery1 then
			...
			while mQuery2 do
				...
				if row2 then
					table.insert(result2, { row2["date"], row2["action"], row2["adminname"], row2["log_id"], row2["intID"]})
				else
					break
				end
			end
			...
			mysql:free_result(mQuery2)
			triggerClientEvent(thePlayer, "createCheckIntWindow", thePlayer, result, exports.cr_global:getPlayerAdminTitle(thePlayer), result2, notes)
		else
...
function restock(thePlayer, commandName, intID, amount)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer) then
		if not intID or not tonumber(intID) or tonumber(intID)%1~=0 then
			...
			intID = getElementDimension(thePlayer)
			if intID == 0 then
				outputChatBox("You must be inside an interior to restock. Or use KULLANIM: /" .. commandName .. " [Interior ID] [Amount 1~300]", thePlayer, 255, 194, 14)
				return false
			end
		else
			if not amount or not tonumber(amount) or tonumber(amount)%1~=0 or tonumber(amount) < 1 or tonumber(amount) > 300 then
				outputChatBox("KULLANIM: /" .. commandName .. " [Interior ID] [Amount 1~300]", thePlayer, 255, 194, 14)
		...
		for _, interior in ipairs(possibleInteriors) do
			if tonumber(intID) == getElementData(interior, "dbid") then
				if not exports.cr_integration:isPlayerAdmin1(thePlayer) then
					local success, msg1, msg2 = exports["cr_job-trucker"]:remoteOrderSupplies(thePlayer, intID, amount, true)
					...
					--exports["cr_interior"]:reloadOneInterior(tonumber(intID))
					...
					if hiddenAdmin == 0 then
						exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. adminTitle .. " " ..  getPlayerName(thePlayer):gsub("_", " ").. " (" .. adminUsername .. ") has restocked " .. getElementData(interior, "name") .. " (ID#" .. intID .. ") with " .. amount .. " of supplies.")
					else
						exports.cr_global:sendMessageToAdmins("[INTERIOR]: Gizli Yetkili has has restocked " .. getElementData(interior, "name") .. " (ID#" .. intID .. ") with " .. amount .. " of supplies.")
					end
...
function setInteriorFaction(thePlayer, cmd, ...)
	if exports.cr_integration:isPlayerAdmin1(thePlayer) then
		...
		if tonumber(clue) then
			theFaction = exports.cr_pool:getElement("team", tonumber(clue))
		else
			theFaction = exports.cr_faction:getFactionFromName(clue)
		end
		...
		local can , reason = exports.cr_global:canFactionBuyInterior(theFaction)
		if not can then
		...
		call(getResourceFromName("cr_items"), "deleteAll", interiorType == 1 and 5 or 4, dbid)
		exports.cr_global:giveItem(thePlayer, interiorType == 1 and 5 or 4, dbid)

		exports.cr_logs:dbLog(thePlayer, 37, { "in" .. tostring(dbid) } , "SETINTFACTION INTERIOR ID#" .. dbid .. " TO FACTION '" .. factionName .. "'")
		exports['cr_interior']:realReloadInterior(tonumber(dbid))
		triggerClientEvent(thePlayer, "createBlipAtXY", thePlayer, entrance[INTERIOR_TYPE], entrance[INTERIOR_X], entrance[INTERIOR_Y])
		exports.cr_global:sendMessageToAdmins("[INTERIOR] " .. exports.cr_global:getPlayerFullIdentity(thePlayer) .. " transferred the ownership of interior '" .. intName .. "' ID #" .. dbid .. " to faction '" .. factionName .. "'.")
		return true
	end
end
...
function setInteriorToMyFaction(thePlayer, cmd)
	...
	local theFaction = exports.cr_pool:getElement("team", tonumber(factionId))
	if not theFaction then
		outputChatBox("No faction found.", thePlayer, 255, 0, 0)
		return
	end
	...
	local can , reason = exports.cr_global:canPlayerFactionBuyInterior(thePlayer)
	if not can then
		outputChatBox(reason, thePlayer, 255, 0, 0)
		return 
	end
	...
	exports.cr_logs:dbLog(thePlayer, 37, { "in" .. tostring(dbid) } , "SETINTTOMYFACTION INTERIOR ID#" .. dbid .. " TO FACTION '" .. factionName .. "'")
	exports['cr_interior']:realReloadInterior(tonumber(dbid))
	triggerClientEvent(thePlayer, "createBlipAtXY", thePlayer, entrance[INTERIOR_TYPE], entrance[INTERIOR_X], entrance[INTERIOR_Y])
	exports.cr_global:sendMessageToAdmins("[INTERIOR] " .. exports.cr_global:getPlayerFullIdentity(thePlayer) .. " transferred the ownership of interior '" .. intName .. "' ID #" .. dbid .. " to his faction '" .. factionName .. "'.")
	return true
end
