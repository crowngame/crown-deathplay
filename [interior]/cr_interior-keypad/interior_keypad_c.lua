...
local numFont = "default-bold-small"
...
function openKeypadInterface(thePad)
	...
	if not thePad or not isElement(thePad) or not isElement(thePad) then
		...
		--exports.cr_hud:sendBottomNotification(localPlayer, "Keyless Digital Door Lock - Fatal Error!", "System was not installed properly, please reinstall the device.")
	else
		currentPad = thePad
	...
	exports.cr_global:centerWindow(GUIEditor_Image[1])
...
function playSyncedSound(code, thePad)
	if code == "doorLockSound" or code == "doorUnlockSound" then
		local sound = playSound3D(":interior-system/" .. code .. ".mp3", thePad[1], thePad[2], thePad[3])
