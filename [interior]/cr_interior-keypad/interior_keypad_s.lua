...
function openKeypadInterface(thePad)
	...
	if not intID then
		--exports.cr_hud:sendBottomNotification(source, "Keyless Digital Door Lock", "System overloaded, try again later.")
		return false
	end
...
function uninstallKeypad(thePad, theInterior)
	...
	if not exports.cr_global:hasSpaceForItem(source, 169, 1) then
	...
	if theInterior and isElement(theInterior) and getElementData(theInterior, "keypad_lock") then
		local count = -1
		for key, thePad2 in pairs(getElementsByType("object", getResourceRootElement(getResourceFromName("cr_item-world")))) do
...
function registerNewPasscode(intID, passcode)
	if mysql:query_free("UPDATE `interiors` SET `keypad_lock_pw`='" .. passcode .. "' WHERE `id`='" .. intID .. "' ") then
...
function resourceStop()
	...
	if getResourceFromName(tostring("item-world")) then
		for key, thePad in pairs(getElementsByType("object", getResourceRootElement(getResourceFromName("cr_item-world")))) do
...
function playerQuit()
	--keypad door lock / Farid
	local padID = getElementData(source, "padUsing")
	if padID then
		for i, thePad in pairs(getElementsByType("object", getResourceRootElement(getResourceFromName("cr_item-world")))) do
...
function playSyncedSound(code, thePad)
	triggerClientEvent(root, "playSyncedSound", source, code, thePad)
end
