...
function ustArama(thePlayer, cmdName, targetPlayer)
	...
	if (logged==1) then
		...
		if not (targetPlayer) then
			outputChatBox("[!] #f0f0f0KULLANIM: /" .. cmdName .. " [Kişi Adı / ID]", thePlayer, 255, 194, 14, true)
		else
			...
			if targetPlayer then
				...
				if (distance<=5) then
					...
					setElementData(thePlayer, "ustArama", true)
				else
...
function aramaKabul(thePlayer, targetPlayer)
	if thePlayer and targetPlayer then
		setElementData(thePlayer, "ustArama", false)
		...
		local marijuana = exports["cr_items"]:countItems(targetPlayer, 38)
		local kokain = exports["cr_items"]:countItems(targetPlayer, 34)
		local mantar = false
		local eroin = exports["cr_items"]:countItems(targetPlayer, 37)
		local ekstazi = exports["cr_items"]:countItems(targetPlayer, 36)
		local meth = exports["cr_items"]:countItems(targetPlayer, 39)
		
		local telefon = exports["cr_items"]:hasItem(targetPlayer, 2)
		local telsiz = exports["cr_items"]:hasItem(targetPlayer, 6)
	
		outputChatBox("[!] #f0f0f0Para: $" .. exports.cr_global:formatMoney(para), thePlayer, 0, 0, 255, true)
...
addEventHandler("pd:aramaKabul", root, aramaKabul)
...
addEventHandler("pd:aramaRed", root, aramaRed)
