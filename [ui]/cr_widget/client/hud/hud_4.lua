    ...
    if getElementData(localPlayer, "loggedin") == 1 then
	    if getElementData(localPlayer, "hud_settings").hud == 4 then
			...
			for key, data in pairs(CIRCLE_VALUES) do
				...
				dxDrawText(text, circularPosition.x, circularPosition.y + CIRCULAR_SIZE, circularPosition.x + CIRCULAR_SIZE, circularPosition.y + CIRCULAR_SIZE + 20, tocolor(217, 217, 217), 1, fonts.regular, "center", "bottom", true, true)
...
function getHudDataValue(key)
	...
	elseif key == "armour" then
        ...
        return armour, math.floor(armour) .. "%"
