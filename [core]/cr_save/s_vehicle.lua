...
function saveVehicle(source)
	...
	if isElement(source) and getElementType(source) == "vehicle" and dbid > 0 then
		...
		--if (owner~=-1) then
			...
			dbExec(mysql:getConnection(), "UPDATE vehicles SET `fuel`='" .. mysql:escape_string(fuel) ..
