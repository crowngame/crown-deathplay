...
function removeBackup(thePlayer, commandName)
	if (exports.cr_integration:isPlayerTrialAdmin(thePlayer)) or (getElementData(getPlayerTeam(thePlayer), "type") == 4 and getElementData(thePlayer, "factionleader") == 1) then
		if (backupPlayer~=nil) then
			
			for k,v in ipairs(getPlayersInTeam (getTeamFromName("Los Santos Medical Department"))) do
				triggerClientEvent(v, "destroyBackupBlip2", root)
			end
...
function destroyBlip()
	...
	for key, value in ipairs(getPlayersInTeam(theTeam)) do
		...
		triggerClientEvent(value, "destroyBackupBlip2", root)
	end
