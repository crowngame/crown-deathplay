...
function atm_render()
    ...
    dxDrawText("",wx+w-45,wy+20,nil,nil,exports.cr_ui:inArea(wx+w-45,wy+20,xw,xh) and tocolor(190,59,59) or tocolor(200,200,200),1,f.awesome)
    if exports.cr_ui:inArea(wx+w-45,wy+20,xw,xh) and getKeyState("mouse1") and click+400 < getTickCount() then
        c_render_close()
        click = getTickCount()
    end
    ...
    for k,v in pairs(list) do
        ...
        if exports.cr_ui:inArea(wx+32+newX-5,wy+139,tw+15,40) and getKeyState("mouse1") and click+400 < getTickCount() then
            selected = k
            click = getTickCount()
        end
        newX = newX + tw + 20
    end 
    if exports.cr_ui:inArea(wx+39, wy+232, w-78, 37) and getKeyState("mouse1") and click+400 < getTickCount() then
        if selected == 1 then
            if aktif == 0 then
                aktif = 1
    ...
    if exports.cr_ui:inArea(wx+39, wy+380, w-78, 42) and getKeyState("mouse1") and click+400 < getTickCount() then
