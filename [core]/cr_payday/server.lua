	...
	for _, player in ipairs(getElementsByType("player")) do
		if getElementData(player, "loggedin") == 1 then
			...
			if minutesPlayed >= 60 then
				...
				local boxHours = getElementData(player, "boxHours") or 0
				local hoursPlayed = getElementData(player, "hoursplayed") or 0
				
				setElementData(player, "boxHours", boxHours + 1)
				setElementData(player, "minutesPlayed", 0)
				setElementData(player, "hoursplayed", hoursPlayed + 1)
				exports.cr_pass:addMissionValue(player, 8, 1)
				
				dbExec(mysql:getConnection(), "UPDATE characters SET boxHours = ? WHERE id = ?", getElementData(player, "boxHours"), getElementData(player, "dbid"))
				...
				if getElementData(player, "boxHours") >= 4 then
					local boxCount = getElementData(player, "boxCount") or 0
					outputChatBox("[!]#FFFFFF Dört saat oynadınız ve bir kutu kazandınız.", player, 0, 255, 0, true)
					setElementData(player, "boxHours", 0)
					setElementData(player, "boxCount", boxCount + 1)
					dbExec(mysql:getConnection(), "UPDATE characters SET boxHours = ? WHERE id = ?", 0, getElementData(player, "dbid"))
					dbExec(mysql:getConnection(), "UPDATE characters SET boxCount = ? WHERE id = ?", getElementData(player, "boxCount"), getElementData(player, "dbid"))
				end
		...
		if controlSystem[thePlayer] then
			if tostring(controlSystem[thePlayer].code) == tostring(code) then
				...
				if getElementData(thePlayer, "vip") == 1 then
					setElementData(thePlayer, "bankmoney", bankMoney + 30000)
					outputChatBox("[!]#FFFFFF Tebrikler, VIP [1] olduğunuz için saatlik maaşınız 30,000$ kazandınız.", thePlayer, 0, 255, 0, true)
				elseif getElementData(thePlayer, "vip") == 2 then
					setElementData(thePlayer, "bankmoney", bankMoney + 40000)
					outputChatBox("[!]#FFFFFF Tebrikler, VIP [2] olduğunuz için saatlik maaşınız 40,000$ kazandınız.", thePlayer, 0, 255, 0, true)
				elseif getElementData(thePlayer, "vip") == 3 then
					setElementData(thePlayer, "bankmoney", bankMoney + 50000)
					outputChatBox("[!]#FFFFFF Tebrikler, VIP [3] olduğunuz için saatlik maaşınız 50,000$ kazandınız.", thePlayer, 0, 255, 0, true)
				elseif getElementData(thePlayer, "vip") == 4 then
					setElementData(thePlayer, "bankmoney", bankMoney + 60000)
					outputChatBox("[!]#FFFFFF Tebrikler, VIP [4] olduğunuz için saatlik maaşınız 60,000$ kazandınız.", thePlayer, 0, 255, 0, true)
				elseif getElementData(thePlayer, "vip") == 5 then
					setElementData(thePlayer, "bankmoney", bankMoney + 70000)
					outputChatBox("[!]#FFFFFF Tebrikler, VIP [5] olduğunuz için saatlik maaşınız 70,000$ kazandınız.", thePlayer, 0, 255, 0, true)
				else
					setElementData(thePlayer, "bankmoney", bankMoney + 20000)
					outputChatBox("[!]#FFFFFF Tebrikler, saatlik maaşınız 20,000$ yatırıldı.", thePlayer, 0, 255, 0, true)
				end
...
function boxRemaining(source, commandName)
	...
	local boxHours = getElementData(source, "boxHours") or 0
	local boxCount = getElementData(source, "boxCount") or 0
...
function openBox(source, commandName)
	if isElementWithinColShape(source, colSphere) then
		local boxCount = getElementData(source, "boxCount") or 0
		if boxCount > 0 then
			local randomMoney = math.random(10000, 50000)
			...
			setElementData(source, "boxCount", boxCount - 1)
			dbExec(mysql:getConnection(), "UPDATE characters SET boxCount = ? WHERE id = ?", getElementData(source, "boxCount"), getElementData(source, "dbid"))
		else
