...
Interaction.Panel = function(interactions,durum)
    if interactionActive then
        ...
        if getElementType(selectedElement) == "vehicle" then
			...
			local model = getElementData(selectedElement, "maximemodel") or getVehicleNameFromModel(getElementModel(selectedElement))
        ...
        dxDrawRectangle(panelX, panelY, panelW, panelH + (rowH * adjustH), tocolor(23, 23, 23, 240))
        ...
        if getElementType(selectedElement) == "vehicle" then
			...
			local yil = getElementData(selectedElement, "year") or "2015"
			local araba = marka .. " " .. model .. " " .. yil 
            dxDrawText(araba, panelX, panelY + respc(10), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto14, "center", "top")
        ...
        elseif getElementType(selectedElement)=="ped"then
            dxDrawText(getElementData(selectedElement, "name") or "İsimsiz", panelX, panelY + respc(10), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto14, "center", "top")
        ...
        if getElementType(selectedElement) == "vehicle" then
            dxDrawText("(Araba)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
        elseif selectedElement == localPlayer then
            dxDrawText("(Kullanıcı Arayüzü)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
		elseif getElementType(selectedElement) == "player" then
            dxDrawText("(Oyuncu)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
        elseif getElementType(selectedElement)=="ped" then
        ...
        elseif getElementType(selectedElement) == "object" then
            dxDrawText("(Obje)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
        end
        ...
        for i = 1, adjustH do
            ...
            if interaction then
	            ...
	            if i % 2 == 0 then 
	                ...
	                dxDrawInteractionButton("interaction:" .. i, interaction[1], rowX, rowY, calculatedRowW, rowH, {45, 45, 45, 50}, {70, 70, 70, 200}, {255, 255, 255}, fonts.Roboto11, "left", "center", interaction[2], iconW, iconH, {100, 100, 100})
	            else
	               
	                dxDrawInteractionButton("interaction:" .. i, interaction[1], rowX, rowY, calculatedRowW, rowH, {45, 45, 45, 50}, {70, 70, 70, 100}, {255, 255, 255}, fonts.Roboto11, "left", "center", interaction[2], iconW, iconH, {100, 100, 100})
	            end
	        end
...
addCommandHandler("home",function()
	if not getElementData(localPlayer, "cam:basladi") then
			Interaction.Show(localPlayer, "home")
	else
		outputChatBox("[!]#FFFFFF Şuanda bu eylemi gerçekleştiremezsinz.",255,0,0,true)
	end
end)
...
addEventHandler("onClientClick", root, function(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
    if button == "right" and state == "down" then
        if not interactionActive then
			...
			exports.cr_interface:showAlert("error", "Var olan bir panel açık.")
        ...
        if interactionActive then
            for i = 1, #elementInteractions do
                if activeButton == "interaction:" .. i then
                    playSound(":assets/audio/interface/click.ogg")
