...
addEventHandler("onClientCustomAnimationUpdate", root, function(player, index)
	dbExec(exports.cr_mysql:getConnection(), "UPDATE characters SET customanim = '" .. index .. "' WHERE id = '" .. getElementData(player, "dbid") .. "'")
end)

function setPlayerCustomAnimations(p, index)
	triggerClientEvent(root, "setPlayerCustomAnimation", root, p, index)
end
