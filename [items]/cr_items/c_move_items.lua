	...
	if getElementType(element)=="interior" then
		...
		if not hasKey(getElementData(element, "dbid"), element) and interiorType~=2 then
			...
			if interiorType==2 then
				if (not exports.cr_faction:isInFactionType(getLocalPlayer(), 2) and not exports.cr_faction:isInFactionType(getLocalPlayer(), 3) and getElementData(getLocalPlayer(), "duty_admin") ~= 1) then
					local row = guiGridListAddRow(gElementItems)
...
function hasKey(key, element)
	...
	elseif getElementData(getLocalPlayer(), "duty_admin") == 1 then
		...
		if interiorType==2 then
			if (not exports.cr_faction:isInFactionType(getLocalPlayer(), 2) and not exports.cr_faction:isInFactionType(getLocalPlayer(), 3)) then
				return false, false
			else
				return true, true
			end
...
local function adminCheck(button)
	...
	if button == "right" and col ~= -1 and row ~= -1 then
		...
		if exports.cr_global:isStaffOnDuty(getLocalPlayer()) then
