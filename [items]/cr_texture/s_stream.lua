...
addEventHandler("frames:delete", resourceRoot,
	function(id)
		...
		if global:hasItem(client, 4, interior) or global:hasItem(client, 5, interior) or (integration:isPlayerAdmin1(client) and global:isAdminOnDuty(client)) or (interior==0) then
			...
			if not data or not data[id] then
				...
				if success then
					...
					exports['cr_items']:giveItem(client, textureItemID, tostring(thisData.url) .. ";" .. tostring(thisData.texture))
...
function newTexture(source, url, texture)
	...
	if dimension >= 1 or (integration:isPlayerSeniorAdmin(source) and global:isAdminOnDuty(source)) or integration:isPlayerScripter(source) then
		if global:hasItem(source, 4, dimension) or global:hasItem(source, 5, dimension) or (integration:isPlayerAdmin1(source) and global:isAdminOnDuty(source)) or (dimension==0) then
