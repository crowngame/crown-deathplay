			...
			if exports.cr_global:hasMoney(source, item[2]) then
				if exports.cr_items:hasSpaceForItem(source, item[1], item[3]) then
					if item[1] == 115 then
						...
						outputChatBox("[!]#FFFFFF Başarıyla $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında " .. getWeaponNameFromID(item[3]) .. " aldınız.", source, 0, 255, 0, true)
						exports.cr_discord:sendMessage("shop-log", "[SHOP] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında " .. getWeaponNameFromID(item[3]) .. " aldı.")
					elseif item[1] == 116 then
						exports.cr_global:takeMoney(source, item[2])
						exports.cr_global:giveItem(source, 116, item[3] .. ":" .. item[4] .. ":" .. getWeaponNameFromID(item[3]) .. " Mermisi")
						outputChatBox("[!]#FFFFFF Başarıyla $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında " .. item[4] .. " mermilik " .. getWeaponNameFromID(item[3]) .. " mermisi aldınız.", source, 0, 255, 0, true)
						exports.cr_discord:sendMessage("shop-log", "[SHOP] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında " .. item[4] .. " mermilik " .. getWeaponNameFromID(item[3]) .. " mermisi aldı.")
					elseif item[1] == 56 then 
						if getElementData(source, "vip") > 0 then 
							exports.cr_global:takeMoney(source, item[2])
							exports.cr_items:giveItem(source, item[1], item[3])
							outputChatBox("[!]#FFFFFF Başarıyla $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında Maske aldınız.", source, 0, 255, 0, true)
							exports.cr_discord:sendMessage("shop-log", "[SHOP] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında Maske aldı.")
						else
						...
						outputChatBox("[!]#FFFFFF Başarıyla $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında " .. exports.cr_items:getItemName(item[1]) .. " aldınız.", source, 0, 255, 0, true)
						exports.cr_discord:sendMessage("shop-log", "[SHOP] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu $" .. exports.cr_global:formatMoney(item[2]) .. " karşılığında " .. exports.cr_items:getItemName(item[1]) .. " aldı.")
					end
				else
...
function shop.prototype.create(self, thePlayer, commandName, type, skin, name)
    if exports.cr_integration:isPlayerLeaderAdmin(thePlayer) then
        if not tonumber(type) or not tonumber(skin) or not name or tonumber(type) > #types or tonumber(type) < 0 then
            ...
            outputChatBox("[!]#FFFFFF Tip: 5 = Silah Mağazası", thePlayer, 0, 0, 255, true)
            outputChatBox("[!]#FFFFFF Tip: 6 = Mermi Mağazası", thePlayer, 0, 0, 255, true)
        else
...
function storeKeeperSay(thePlayer, content, pedName)
	...
	exports.cr_global:sendLocalText(thePlayer, tostring(pedName) .. ": " .. content, 255, 255, 255, 30, {}, true)
end
addEvent("shop:storeKeeperSay", true)
addEventHandler("shop:storeKeeperSay", root, storeKeeperSay)
