...
function shop.prototype.render(self)
	...
	self.timer = setTimer(function()
		...
		dxDrawText("", self.screenX + self.sizeX - 40, self.screenY + 20, nil, nil, exports.cr_ui:inArea(self.screenX + self.sizeX - 40, self.screenY + 20, dxGetTextWidth("", 1, self.fonts.awesome), dxGetFontHeight(1, self.fonts.awesome)) and tocolor(234, 83, 83, 255) or tocolor(255, 255, 255, 255), 1, self.fonts.awesome)
		if exports.cr_ui:inArea(self.screenX + self.sizeX - 40, self.screenY + 20, dxGetTextWidth("", 1, self.fonts.awesome), dxGetFontHeight(1, self.fonts.awesome)) and getKeyState("mouse1") and self.clickTick + 500 < getTickCount() then
		...
		for key, value in pairs(shopItems[self.show]) do
			if key > self.scroll and self.countY < self.maxItem then
				dxDrawRectangle(self.screenX + 20 + self.newX, self.screenY + 20 + self.newY + 75, 85, 85, exports.cr_ui:inArea(self.screenX + 20 + self.newX, self.screenY + 20 + self.newY + 75, 85, 85) and tocolor(50, 50, 50, 200) or tocolor(25, 25, 25, 200))
				if value[1] == 115 then
				...
				dxDrawText("$" .. exports.cr_global:formatMoney(value[2]), self.screenX + 75 + self.newX, self.screenY + self.newY + 160, self.screenX + 50 + self.newX, nil, tocolor(180, 180, 180, 255), 1, self.fonts.light, "center")
				...
				if exports.cr_ui:inArea(self.screenX + 20 + self.newX, self.screenY + 20 + self.newY + 75, 85, 85) and getKeyState("mouse1") and self.clickTick + 500 < getTickCount() then
					self.clickTick = getTickCount()
...
function shop.prototype.up(self)
    if isTimer(self.timer) then 
        if exports.cr_ui:inArea(self.screenX, self.screenY, self.sizeX, self.sizeY) then 
