						...
						if targetPlayer then
							...
							if minutes > 0 then
								...
								if playerLevel >= targetLevel then
									...
									dbQuery(function(queryHandler, thePlayer, targetPlayer, minutes, reason, serial)
										local result, rows, err = dbPoll(queryHandler, 0)
										if (rows > 0) and (result[1]) then
											data = result[1]
											if data then
												outputChatBox("[!]#FFFFFF Zaten [" .. data.serial .. "] serial'lı kullanıcı sunucudan yasaklı durumda.", thePlayer, 255, 0, 0, true)
											...
											local endTick = (math.floor(minutes) >= 5000) and -1 or math.floor(minutes) * 60 * 1000
											...
											if math.floor(minutes) >= 5000 then
												outputChatBox("[CBAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. getPlayerName(targetPlayer):gsub("_", " ") .. " (" .. (getElementData(targetPlayer, "account:username") or "?") .. ") isimli oyuncuyu sunucudan yasakladı.", root, 255, 0, 0)
												outputChatBox("[CBAN] Sebep: " .. reason .. " (Sınırsız)", root, 255, 0, 0)
												exports.cr_discord:sendMessage("ban-log", "[CBAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. getPlayerName(targetPlayer):gsub("_", " ") .. " (" .. (getElementData(targetPlayer, "account:username") or "?") .. ") isimli oyuncuyu sunucudan yasakladı.")
											...
											addAdminHistory(targetPlayer, thePlayer, reason, 7, (tonumber(math.floor(minutes)) and (math.floor(minutes) >= 5000 and 0 or minutes) or 0))
											...
											if isElement(targetPlayer) then
												...
												setPlayerName(targetPlayer, "CDP." .. getElementData(targetPlayer, "playerid"))
									...
									end, {thePlayer, targetPlayer, minutes, reason, serial}, mysql:getConnection(), "SELECT serial FROM bans WHERE serial = ?", serial)
								else
				...
				if minutes then
					if (...) then
						...
						if string.len(serial) == 32 then
							if minutes > 0 then
								dbQuery(function(queryHandler, thePlayer, serial, minutes, reason)
									local result, rows, err = dbPoll(queryHandler, 0)
									if (rows > 0) and (result[1]) then
										data = result[1]
										if data then
										...
										if math.floor(minutes) >= 5000 then
											...
											exports.cr_discord:sendMessage("ban-log", "[OCBAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili [" .. serial .. "] serialini sunucudan yasakladı.")
											exports.cr_discord:sendMessage("ban-log", "[OCBAN] Sebep: " .. reason .. " (Sınırsız)")
										else
											outputChatBox("[OCBAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili [" .. serial .. "] serialini sunucudan yasakladı.", root, 255, 0, 0)
											outputChatBox("[OCBAN] Sebep: " .. reason .. " (" .. math.floor(minutes) .. " dakika)", root, 255, 0, 0)
											exports.cr_discord:sendMessage("ban-log", "[OCBAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili [" .. serial .. "] serialini sunucudan yasakladı.")
											exports.cr_discord:sendMessage("ban-log", "[OCBAN] Sebep: " .. reason .. " (" .. math.floor(minutes) .. " dakika)")
										end
								...
								outputChatBox("[!]#FFFFFF Geçerli bir sayısal dakika giriniz.", thePlayer, 255, 0, 0, true)
								playSoundFrontEnd(thePlayer, 4)
							end
						else
							outputChatBox("[!]#FFFFFF Geçerli bir serial giriniz.", thePlayer, 255, 0, 0, true)
							playSoundFrontEnd(thePlayer, 4)
						end
					else
					...
					if minutes then
						if (...) then
							...
							if minutes >= 0 then
								...
								if playerLevel >= targetLevel then
									if minutes == 0 then
										outputChatBox("[BAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. getPlayerName(targetPlayer):gsub("_", " ") .. " (" .. (getElementData(targetPlayer, "account:username") or "?") .. ") isimli oyuncuyu yasakladı.", root, 255, 0, 0)
										outputChatBox("[BAN] Sebep: " .. reason .. " (Sınırsız)", root, 255, 0, 0)
										exports.cr_discord:sendMessage("ban-log", "[BAN] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. getPlayerName(targetPlayer):gsub("_", " ") .. " (" .. (getElementData(targetPlayer, "account:username") or "?") .. ") isimli oyuncuyu sunucudan yasakladı.")
									...
									banPlayer(targetPlayer, true, false, true, getElementData(thePlayer, "account:username"), reason, minutes * 60)
					...
					if (...) then
						...
						if string.len(serial) == 32 then
							if minutes >= 0 then
								...
								addBan(nil, nil, serial, getElementData(thePlayer, "account:username"), reason, minutes * 60)
			...
			if targetPlayer then
				...
				if targetPlayer then
					if reason then
						...
						local targetLevel = getElementData(targetPlayer, "admin_level") or 0
						if playerLevel >= targetLevel then
							outputChatBox("[KICK] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. getPlayerName(targetPlayer):gsub("_", " ") .. " (" .. (getElementData(targetPlayer, "account:username") or "?") .. ") isimli oyuncuyu sunucudan attı.", root, 255, 0, 0)
							outputChatBox("[KICK] Sebep: " .. reason, root, 255, 0, 0)
							exports.cr_discord:sendMessage("kick-log", "[KICK] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. getPlayerName(targetPlayer):gsub("_", " ") .. " (" .. (getElementData(targetPlayer, "account:username") or "?") .. ") isimli oyuncuyu sunucudan attı.")
							exports.cr_discord:sendMessage("kick-log", "[KICK] Sebep: " .. reason)
							
							kickPlayer(targetPlayer, getElementData(thePlayer, "account:username"), reason)
