...
function checkVeh(thePlayer, commandName, vehID)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer) then
		...
		if mQuery1 then  
			...
			mysql:free_result(mQuery2)
			triggerClientEvent(thePlayer, "createCheckVehWindow", thePlayer, result, exports.cr_global:getPlayerAdminTitle(thePlayer), result2, notes)
		else
...
function getVehicleWeight (thePlayer, commandName, specifiedVehicle)
	if exports.cr_integration:isPlayerVehicleConsultant(thePlayer) or exports.cr_integration:isPlayerAdmin1(thePlayer) or exports.cr_integration:isPlayerVCTMember(thePlayer) then
		...
		elseif tonumber(specifiedVehicle) and tonumber(specifiedVehicle) > 0 then
			local exists = exports.cr_pool:getElement("vehicle", tonumber(specifiedVehicle)) or false
			if not exists then
...
function systemDeleteVehicle(vehid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the delveh however, it's simplier and doesn't use mta elements. / Farid / 2015.1.11
	...
	call(getResourceFromName("cr_items"), "deleteAll", 3 , vehid)
	...
	exports.cr_logs:dbLog("SYSTEM", 6, { "veh" .. vehid } , reason and reason or "DELETED")
	addVehicleLogs(vehid, reason and reason or "DELETED BY SYSTEM")

	--Check if interior is loaded in game
	local theVehicle = exports.cr_pool:getElement("vehicle", vehid)
	if theVehicle then
